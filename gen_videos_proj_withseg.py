...
def gen_interp_video(G, mp4: str, ws, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, shape_format='.mrc', camera_up=0, device=torch.device('cuda'), **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
    ...
    for frame_idx in tqdm(range(num_keyframes * w_frames)):
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                if cfg == "Head":
                    cam2world_pose = LookAtPoseSampler.sample(3.14/2 + 2 * 3.14 * frame_idx / (num_keyframes * w_frames), 3.14/2+camera_up,
                ...
                if gen_shapes and frame_idx == 0:
                    ...
                    if output_ply:
                        ...
                        #convert_sdf_samples_to_ply(np.transpose(sigmas, (2, 1, 0)), [0, 0, 0], 1, os.path.join(outdirs, mp4.replace('.mp4', '.ply')), level=10)  #directory path wrong
                    else: # output mrc
...
@click.option('--camera-up', help='Camera up', type=float, default=0)
...
def generate_images(
    ...
    gen_interp_video(G=G, mp4=output, ws=ws, bitrate='100M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=w_frames, psi=truncation_psi, truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode, gen_shapes=shapes, shape_format=shape_format, camera_up=camera_up, device=device)
