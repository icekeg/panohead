...
def generate_images(
    ...
    #PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/output.png')
    ...
    if shapes:
        ...
        w = torch.from_numpy(np.array((1.2))).to(device)#????  
        ...
        with tqdm(total = samples.shape[1]) as pbar:
            with torch.no_grad():
                while head < samples.shape[1]:
                    torch.manual_seed(0)
                    sigma = G.sample(samples[:, head:head+max_batch], transformed_ray_directions_expanded[:, :samples.shape[1]-head], w.unsqueeze(0), conditioning_params, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, noise_mode='const')['sigma']
        ...
        if shape_format == '.ply':
            ...
            convert_sdf_samples_to_ply(np.transpose(sigmas, (2, 1, 0)), [0, 0, 0], 1, os.path.join(outdir, f'output.ply'), level=10)
        elif shape_format == '.mrc': # output mrc
            with mrcfile.new_mmap(os.path.join(outdir, f'output.mrc'), overwrite=True, shape=sigmas.shape, mrc_mode=2) as mrc:
                mrc.data[:] = sigmas
